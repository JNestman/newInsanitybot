...
namespace insanitybot
{
	...
	class InformationManager
	{
		...
		void checkForDeadWorkers();

		// update() sub-routines (extracted from monolithic update)
		void updateAggression();
		void updateNukeDotTracking();
		void resolveEnemyBaseLocations();
		void updateInitialScouting();
		void updateRushDetection();
		void cleanDeadUnits();
		void tryRegisterAllUnits();
		void pruneEnemyStructures();
		void checkLostEnemyBases();
		void updateScans();
		void cleanZombieTasks();

		// Unit registration helpers (extracted from tryRegisterAllUnits)
		void registerBuildingStates(BWAPI::Unit unit);
		bool tryRegisterWorker(BWAPI::Unit unit);
		void assignWorkerToBase(BWAPI::Unit unit);
		bool tryRegisterCombatUnit(BWAPI::Unit unit);
		bool tryRegisterBuilding(BWAPI::Unit unit);

		// Build order dispatch table
		using BuildOrderFn = std::function<void(BuildOrder&, InformationManager&)>;
		std::unordered_map<std::string, BuildOrderFn>		_buildOrderDispatch;
		void initBuildOrderDispatch();
	...
	public:
		...
		bool shouldExpand();
		bool isExpanding();
		bool expansionOutsideOfMainAndNatConstructing() { return (getNumUnfinishedUnit(BWAPI::UnitTypes::Terran_Command_Center) && _ownedBases.size() > 2); };
		...
		bool shouldIslandExpand() { return false; }; // TODO: Work out drops/island expansion hiccups
